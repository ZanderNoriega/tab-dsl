...
export const note = (length: Length) => <T>(x: T): Note<T> => ({ length, ...x })
...
export const eighths    = (xs: readonly Fret[]): Group<Guitar> => group(xs.map(eighth));
export const sixteenths = (xs: readonly Fret[]): Group<Guitar> => group(xs.map(sixteenth));
